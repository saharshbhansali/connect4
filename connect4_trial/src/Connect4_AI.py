...
# Human and AI tokens taken to help us check win conditions, and 1 and 2 can be used while testing
#while len(Human)>8 or len(AI)>8 or len(Human)<1 or len(AI)<1 or Human == AI:
# Human = 'X' 
# AI = 'O'
...
main_board = {} 
# board is a dictionary for easy access of each column using numbers 1 - 7 as keys
# print(new_board()) --- test code to check if board is working
# display()
main_board = new_board(main_board)
winner = win_condition()
opponent_choice = ''
...
while opponent_choice not in ('1', '2','3'):
...
while not winner:
    ...
    if opponent_choice in ('2', '3', '4') and turn == 0: pass
    else:
    ...
    elif menu.upper() == 'UNDO' and opponent_choice == '2' and  starter == 0 and len(move_list) <= 1:
        print('Cannot undo, AI going first.')

    elif menu.upper() == 'UNDO' and opponent_choice == '2': 
        undo(Human, AI)
        undo(AI, Human)
        display()
        time.sleep(1)
    ...
    #    if len(move_list) >= 0: turn = (turn+1)%2
        ...
        try:
            ...
            if turn == 0 and not win_condition():
