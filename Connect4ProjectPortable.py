...
def SelectAllPlayerStats():
    ...
    for head in col_heads:
        if head == "Total Games":
            print("| %12s" % head, "%2s" % end, end="")
        else:
            print("| %8s" % head, "%6s" % end, end="")
    print()
...
def PlayerLogin():
    ...
    if a and bool(a) == True:
        ...
        print("Player Name does not exist. Please Sign-In first.")
...
def UpdatePlayerStats(win_player, loss_player, draw=False):
    ...
    try:
        if draw == True and win_player != loss_player:
            cursor_obj.execute(
                f"update userdata set pTotal = pTotal + 1, pDraws = pDraws + 1 where pName in ('{win_player}','{loss_player}');"
            )
            print("Successfully updated data.")
        elif draw == False and win_player != loss_player:
            cursor_obj.execute(
                f"update userdata set pTotal = pTotal + 1, pWins = pWins + 1 where pName = '{win_player}';"
            )
            cursor_obj.execute(
                f"update userdata set pTotal = pTotal + 1, pLosses = pLosses + 1 where pName = '{loss_player}';"
            )
            print("Successfully updated data.")
        else:
            print("Error while updating data. Contact Admin for help.")
...
def Connect4_2PlayerGame():
    def new_board(board={}):
        board = {
            i: [i if j == ROW_Count else "---" for j in range(ROW_Count + 1)]
    ...
    def win_condition():
        ...
        for i in range(ROW_Count - Plot_What + 1):
            for j in range(COL_Count):
                ...
                if col.count(Human) == Plot_What:
                    return f"Player 1 ({Human}) Wins!"
                elif col.count(AI) == Plot_What:
                    return f"Player 2 ({AI}) Wins!"

        for i in range(ROW_Count):
            for j in range(COL_Count - Plot_What + 1):
                row = [main_board[k][i] for k in range(j, j + Plot_What)]
                if row.count(Human) == Plot_What:
                    return f"Player 1 ({Human}) Wins!"
                elif row.count(AI) == Plot_What:
                    return f"Player 2 ({AI}) Wins!"
        ...
        for i in range(ROW_Count - Plot_What + 1):
            for j in range(COL_Count - Plot_What + 1):
                ...
                if up_diag.count(Human) == Plot_What:
                    return f"Player 1 ({Human}) Wins!"
                elif up_diag.count(AI) == Plot_What:
                    return f"Player 2 ({AI}) Wins!"
        ...
        for i in range(Plot_What - 1, ROW_Count):
            for j in range(COL_Count - Plot_What + 1):
                ...
                if down_diag.count(Human) == Plot_What:
                    return f"Player 1 ({Human}) Wins!"
                elif down_diag.count(AI) == Plot_What:
                    return f"Player 2 ({AI}) Wins!"
    ...
    def check_legal():
        ...
        if ord(move[0]) < 48 or ord(move[0]) > 57 or len(move) == 0:
            ...
            while (
                int_move < 0
                or int_move > COL_Count - 1
                or main_board[int_move].count("---") == 0
            ):
                if int_move in range(7):
...
if (main_cursor, main_connection) == (None, None):
    cont = "no"
else:
    CloseCursorAndConnection(main_cursor, main_connection)
            ...
            while sub_menu not in ("1", "2", "3", "4", "5") or sub_over == False:
                ...
                if sub_menu == "1":
                    ...
                    if bool(player1) == False:
                        ...
                        print(
                            "Two players already logged-in. Log-out of one account to log-in."
                        )
                    ...
                    print(
                        "This option needs 2 players to be logged in. \n\nPlease Log-In and RETRY."
                    )
                    time.sleep(5)

                elif (
                    sub_menu == "3"
                    and (bool(player1) == bool(player2) == True)
                    and player1 != player2
                ):
                    ...
                    if result == "Player 1 (X) Wins!":
                        UpdatePlayerStats(
                            win_player=player1, loss_player=player2, draw=False
                        )

                    elif result == "Player 2 (O) Wins!":
                        UpdatePlayerStats(
                            win_player=player2, loss_player=player1, draw=False
                        )

                    elif result == "It is a Draw!":
                        UpdatePlayerStats(
                            win_player=player1, loss_player=player2, draw=True
                        )
